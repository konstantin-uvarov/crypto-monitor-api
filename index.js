...
auth().signInWithEmailAndPassword(process.env.FIREBASE_USER_NAME, process.env.FIREBASE_USER_PASSWORD)
  .then((userCredential) => {
    // Signed in
    ...
    try {
      ...
      console.log('TEST API message added');
    } catch (error) {
  ...
  userRef.once('value').then(function(snapshot) {
    ...
    if (email) {
      sendNotificationEmail(email).then(function() {
        ...
        // [START write_fan_out]
        const update = {};
        update['chats/' + postId + '/lastNotificationTimestamp'] =
...
function startListeners() {
  /* firebase.database().ref('chats').on('value', function(postSnapshot) {
    ...
    postReference.child('stars').on('value', function(dataSnapshot) {
      ...
      console.log('Failed to add "value" listener at chats/' + postId + '/stars node:', error);
    });
    ...
    postReference.child('stars').on('child_added', function(dataSnapshot) {
      ...
      console.log('Failed to add "child_added" listener at chats/' + postId + '/stars node:', error);
    });
    // [END child_event_listener_recycler]
  }); */
  db.ref("chats").on("value", snapshot => {
    snapshot.forEach((snap) => {
      console.log('snapshot.forEach: ', snap.val());
    });
  });
...
function startWeeklyTopPostEmailer() {
  ...
  schedule.scheduleJob({hour: 14, minute: 30, dayOfWeek: 0}, function () {
    ...
    const topPostsRef = firebase.database().ref('chats').orderByChild('starCount').limitToLast(5);
    // [END top_posts_query]
